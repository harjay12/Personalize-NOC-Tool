...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        try:
            self.ui.comboBox.addItems([x for x in WorkToolFunc.conductors])
            ...
            self.ui.CCABBRcomboBox.addItems(
                sorted([x.upper() for x in WorkToolFunc.CarrierPH])
            )
    ...
    def on_nsoidStr(self):
        ...
        if nsoidStr.startswith("CPEGRT"):
            WorkToolFunc.ahkExe_Path().run_script(
                f'Run "{WorkToolFunc.conductors[self.statusLink]}routers/{nsoidStr}"'
            )

        else:
            WorkToolFunc.ahkExe_Path().run_script(
                f'Run "{WorkToolFunc.conductors[self.statusLink]}"'
            )
            time.sleep(2.5)
    ...
    def CarrierCalls(self):
        ...
        if self.ui.CCABBRcomboBox.currentIndex() != 0:
            num = WorkToolFunc.CarrierPH[self.phoneNm.lower()]
